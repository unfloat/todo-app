...
function initDatabaseConnection() {
  // Use global test database path if available (for testing)
  if (global.testDbPath) {
    db = new sqlite3.Database(global.testDbPath, (err) => {
      if (err) {
        console.error('Error opening test database:', err.message);
      } else {
        console.log('Using test database instance.');
        initDatabase();
      }
    });
  } else {
...
// Function to set database path (for testing)
function setTestDatabasePath(testDbPath) {
  global.testDbPath = testDbPath;
  console.log('Test database path set.');
}
...
module.exports = { app, setTestDatabasePath }; 